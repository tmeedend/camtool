...
| 9 | L'historique de l'extra car (50 positions, pour l'avance de `OFF TRACKING`) n'est tenu que **pendant que `MIX` > 0** (`InterpolateFrame.py` ~253) | `MIX` qui remonte au-dessus de 0 | l'avance est calculée sur un historique périmé, ou rempli de zéros la première fois : visée faussée pendant ~50 frames, même famille que #16. **Non reproduit**, écart voulu : le portage tient l'historique à chaque frame, et en recommence un quand l'extra car change | `test_mix.lua` |
| 10 | Extra car = voiture suivie (le cas par défaut, les deux valant 0) : `calculate_cam_rot_to_tracking_car` est appelée deux fois par frame pour la même voiture | `MIX` > 0 sans extra car choisie | l'historique de la voiture suivie avance de deux pas par frame, ce qui change un peu son avance alors que le mélange ne change rien. **Non reproduit** : le portage ne mélange pas une voiture avec elle-même | `test_mix.lua` |
| 11 | Mouse look compté en **frames** : 60 échantillons, freinage 0.95 et rotation appliqués par frame (`MouseLook.py`) | toute machine qui ne tourne pas à 60 FPS | à 144 FPS, le même geste tourne la caméra ~2,4 fois plus loin et l'élan dure 0,4 s. **Non reproduit**, tranché par Théo : horloge fixe à 60 Hz, identique à CamTool 2 à 60 FPS | `test_mouselook.lua` |
| 12 | L'adoucissement du zoom démarre à « touche tout juste relâchée » (`__t_zoom = 0`, mode `"in"`) | chargement de l'app, caméra libre | la caméra libre zoome de plusieurs degrés pendant le premier quart de seconde. **Non reproduit** : le zoom démarre au repos | `test_mouselook.lua` |
...
- **#16 glissement — UNE cause confirmée, PAS toute l'issue.** `Camera.py`
  ...
  la direction opposée. Théo a comparé les deux en jeu avec la case à cocher du
  POC : **le glissement apparaît avec le remplissage à zéro et disparaît
  sans.** Chiffré depuis, hors jeu : **20,5° sur 0,82 s** pour une caméra de
  bord de piste, et `tests/test_trace_replay.lua` mesure **1,56 rad (89°)** sur
  une vraie session enregistrée.

  ⚠️ **CamTool 3 ne le reproduit plus** (`legacyZeroFill` n'est plus piloté par
  le mode du fichier), **et Théo voit toujours un glissement**, de façon
  intermittente. Donc ce remplissage à zéro était *une* cause et pas *la*
  cause. La suite — pistes rangées et protocole de mesure en jeu — est dans
  `docs/etat.md`, section #16.

  Le second mécanisme trouvé au même endroit : le même historique n'était pas
  vidé **après un saut de replay**, ce qui vaut 1 à 3° selon la géométrie.
  Corrigé aussi. Les deux étaient réels et aucun des deux ne ferme l'issue.
